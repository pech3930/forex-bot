...
{a['summary']}" for a in articles)
    ...
    # พรอมต์ได้รับการปรับปรุงให้บังคับผลลัพธ์เป็นโครงสร้าง JSON เพื่อความแม่นยำในการแกะข้อมูล 100%
...
"""
    ...
    # อัปเดตโมเดลเป็น claude-3-5-sonnet เพื่อความคุ้มค่า ความเร็ว และความแม่นยำสูงสุดในการจัดรูปแบบข้อมูล
...
def parse_result(text, pairs):
    # ปรับปรุงระบบ Parsing ให้เสถียรด้วยการแปลงข้อมูลผ่าน JSON
    # หากเกิดกรณีขัดข้อง ตัวระบบจะใช้โครงสร้างสำรอง (Fallback) โดยอัตโนมัติ พอร์ตไม่ค้างแน่นอน
    try:
        # พยายามค้นหาขอบเขตของ JSON ในกรณีที่ AI เผลอพิมพ์ข้อความอื่นปนมา
        start_idx = text.find('{')
        ...
        # Fallback ในกรณีที่โครงสร้างเสียหาย
...
function drawRoom(){{
  ...
  box(-0.07,3.5,3.12,0.07,2,0.08,'#d0d0c0','#b0b0a0','#c0c0b0');
